...
#今日处理------
getBooks3 <- function(table='t_tsp_ques') {
  sql <- sql_gen_select(conn,table = table)
  #添加内容
...
books.insert.callback3 <- function(data, row ,table='t_tsp_ques',f=getBooks3,id_var='FId') {
  sql_header <- sql_gen_insert(conn,table)
  fieldList <-sql_fieldInfo(conn,table)
  ncount <-nrow(fieldList)
  res <- character(ncount)
...
books.update.callback3 <- function(data, olddata, row,
                                   table='t_tsp_ques',
                                   f=getBooks3,
                                   edit.cols = c('FQues','FAnsw'),
                                   id_var='FId') 
...
books.delete.callback3 <- function(data, row ,table ='t_tsp_ques',f=getBooks3,id_var='FId') {
  sql_header <- sql_gen_delete(table);
  sql_tail <-paste0('  ',id_var,' = ',data[row,id_var])
  query <- paste0(sql_header,sql_tail)
